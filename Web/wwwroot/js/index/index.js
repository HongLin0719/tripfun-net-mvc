...
const app = createApp({
  ...
  methods: {
    ...
    getDateLabel(day) {
      ...
      const trip = this.trips.filter(
        (item) => item.primeKey === this.activeTrip.primeKey
      );
    ...
    addDay() {
      ...
      console.log(this.tripDaysTabs.length);
      this.tripDaysTabs.push({
        label: `Day ${this.tripDaysTabs.length}`,
        name: `day${this.tripDaysTabs.length}`,
      });

      const lastDate = new Date(this.activeTrip.lastDate);
      const newLastDate = lastDate.setDate(lastDate.getDate() + 1);
      const newLastDateStr = `${lastDate.getFullYear()}/${(
        lastDate.getMonth() + 1
      )
        .toString()
      ...
      console.log(newLastDateStr);
      this.activeTrip.lastDate = newLastDateStr;
      console.log(this.activeTrip);
      // endDate.value = newEndDate.toISOString().split("T")[0];
    },
    //以下是Modal方法
    showInfoModal(place, modalName) {
      // 更新模態框的資料
      this.selectedPlace = place;

      // 使用 Bootstrap 方法開啟 Modal
    ...
    showChooseListModal() {
      ...
      const modal = new bootstrap.Modal(
        document.getElementById("chooseListModal")
      );
      modal.show();
    },
    addToWhereModal() {
      // 使用 Bootstrap 方法開啟 Modal
      const modal = new bootstrap.Modal(
        document.getElementById("addToWhereModal")
      );
      modal.show();
    },
  },
  ...
  mounted() {
    ...
    // 監聽所有 .list-group-item.productCard 點擊事件
    // document
    //   .querySelectorAll(".list-group-item.productCard")
    //   .forEach((item) => {
    //     item.addEventListener("click", (event) => {
    //       // 找到對應的景點資料
    //       const index = Array.from(item.parentElement.children).indexOf(item);
    //       this.showModal(this.places[index]);
    //     });
    //   });
  },
